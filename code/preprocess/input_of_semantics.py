...
for dataset in datasets_ch + datasets_en:
    ...
    if dataset in datasets_ch:
        embeddings_index = load_embeddings(
            language="Chinese",
            embeddings_file="../../word-embedding/sgns.weibo.bigram-char",
        )
        CONTENT_WORDS = 100
        EMBEDDING_DIM = 300
    else:
        embeddings_index = load_embeddings(
            language="English",
            embeddings_file="../../word-embedding/glove.twitter.27B.200d.txt",
        )
        CONTENT_WORDS = 50
        EMBEDDING_DIM = 200
    ...
    split_datasets = [
        json.load(
            open(
                os.path.join(data_dir, "{}.json".format(t)),
                "r",
                encoding="utf-8",
            )
        )
    ...
    print(
        "\n{}: {}, len(texts) = {}, \neg: texts[0] = {}\n".format(
            dataset,
            sum([len(d) for d in split_datasets.values()]),
            len(texts),
            texts[0],
        )
    )
    ...
    np.save(
        os.path.join(
            output_dir, "embedding_matrix_{}.npy".format(embedding_matrix.shape)
        ),
        embedding_matrix,
    )
